...
hifleet_containers=fread("~/R/AIS_Processing/carbon-main/data/hifleetships_brief_container_for_student.csv")[,.SD[1,],mmsi][,mmsi:=as.character(mmsi)]
...
# Global average fuel sulfur content in percentage per year 2.6 for HFO Table22
aship=aship[,mainSO2EF:=2*0.97753*2.6][,auxSO2EF:=2*0.97753*0.07][,boilerSO2EF:=2*0.97753*2.6]#4th IMO GHG table 22 主机0.5 低硫
...
#-------------------------------------------------------------------------------------------------
#                                   ****PM10 PM2.5****
#  查找 EFe (其中已经包括SFC计算结果，所以公式16，17可能没用)
PM10_file=fread('~/R/AIS_Processing/carbon-main/data/PM10_EF.csv')
mainPM10_EF=PM10_file[gen==aship[1]$builtYearGroup&
                                  engineType=='MSD'&                        # 中速柴油机
                                  fuelType=='HFO',                          # Heavy Fuel Oil 重油
                                EF]

auxPM10_EF=PM10_file[gen==aship[1]$builtYearGroup&
                             engineType=='Auxiliary engines'&                 # 辅机
                             fuelType=='MDO',                                 # Marine Diesel Oil 船用柴油
                           EF]

boilerPM10_EF=PM10_file[gen==aship[1]$builtYearGroup& 
                                engineType=='Steam Turbines (and boilers)'&   # 锅炉
                                fuelType=='HFO',                              # Heavy Fuel Oil 重油
                              EF]
# EMi = EFe * Wi

aship=aship[,mainPM10EM:=aPower*mainPM10_EF*timespan/3600][,auxPM10EM:=aPower*auxPM10_EF*timespan/3600][,boilerPM10EM:=aPower*boilerPM10_EF*timespan/3600]#单位为克

# PM2.5 assumes it makes up 92% of PM10 (EPA, 2019). P94
aship=aship[,mainPM2.5EM:=0.92*mainPM10EM][,auxPM2.5EM:=0.92*auxPM10EM][,boilerPM2.5EM:=0.92*boilerPM10EM]
...
#-------------------------------------------------------------------------------------------------
#                                   ****CH4****
CH4_file=fread('~/R/AIS_Processing/carbon-main/data/CH4_EF.csv')
mainCH4_EF=CH4_file[engineType=='MSD'&                        # 中速柴油机
                        fuelType=='HFO',                          # Heavy Fuel Oil 重油
                      EF]

auxCH4_EF=CH4_file[engineType=='Auxiliary engines'&                 # 辅机
                       fuelType=='MDO',                                 # Marine Diesel Oil 船用柴油
                     EF]

boilerCH4_EF=CH4_file[engineType=='Steam Turbines (and boilers)'&   # 锅炉
                          fuelType=='HFO',                              # Heavy Fuel Oil 重油
                        EF]

aship=aship[,mainCH4EM:=aPower*mainCH4_EF*timespan/3600][,auxCH4EM:=aPower*auxCH4_EF*timespan/3600][,boilerCH4EM:=aPower*boilerCH4_EF*timespan/3600]#单位为克

#-------------------------------------------------------------------------------------------------
#                                   ****CO****
CO_file=fread('~/R/AIS_Processing/carbon-main/data/CO_EF.csv')
mainCO_EF=CO_file[engineType=='MSD'&                        # 中速柴油机
                      fuelType=='HFO',                          # Heavy Fuel Oil 重油
                    EF]

auxCO_EF=CO_file[engineType=='Auxiliary engines'&                 # 辅机
                     fuelType=='MDO',                                 # Marine Diesel Oil 船用柴油
                   EF]

boilerCO_EF=CO_file[engineType=='Steam Turbines (and boilers)'&   # 锅炉
                        fuelType=='HFO',                              # Heavy Fuel Oil 重油
                      EF]

aship=aship[,mainCOEM:=aPower*mainCO_EF*timespan/3600][,auxCOEM:=aPower*auxCO_EF*timespan/3600][,boilerCOEM:=aPower*boilerCO_EF*timespan/3600]#单位为克
